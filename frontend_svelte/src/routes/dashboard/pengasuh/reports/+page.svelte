<script lang="ts">
  ...
  async function fetchReport() {
    ...
    try {
      ...
      const [resReport, resInv] = await Promise.all([
        pengasuhApi.getReports(reportFilters),
        pengasuhApi.getInvoicesPaginated(invoiceFilters, invPage, invLimit)
      ]);
  ...
  async function fetchInvoices() {
    try {
      ...
      const res = await pengasuhApi.getInvoicesPaginated(invoiceFilters, invPage, invLimit);
  ...
  onMount(async () => {
    try {
      const [dashboardRes, catRes] = await Promise.all([
        pengasuhApi.getDashboard(),
        pengasuhApi.getCategories()
      ]);
...
<div class="space-y-6 flex flex-col flex-1 min-h-0">
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 no-print">
    <div>
      ...
      <p class="text-gray-500 text-sm mt-1">Monitoring laporan pemasukan pesantren secara real-time.</p>
    </div>
  </div>
  ...
  <div id="print-area" class="space-y-6 flex flex-col flex-1 min-h-0">
    ...
    <Card padding={false} class="flex-1 flex flex-col min-h-0 border-l-4 border-l-purple-500">
            ...
            <div>
              ...
              <div class="relative">
                <select
                  ...
                  class="w-full pl-3 pr-8 py-2.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all appearance-none cursor-pointer"
                >
          ...
          <div class="flex items-center gap-3 shrink-0 w-full lg:w-auto justify-end">
            ...
            <div class="relative inline-block text-left w-full sm:w-auto">
              <button 
                ...
                disabled={exporting}
                class="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 text-sm font-bold text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                <Download size={16} />
