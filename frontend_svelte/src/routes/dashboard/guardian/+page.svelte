  ...
  {:else if stats}
    ...
    {#if stats.unpaid_count > 0}
      <div class="bg-amber-50 border border-amber-200 rounded-xl p-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <p class="font-bold text-amber-900 mb-0.5">
          ...
          <a
            ...
            class="flex-shrink-0 inline-flex items-center gap-2 px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg text-sm transition-all shadow-sm"
          >
            Bayar Sekarang
    ...
    {:else}
      <div class="bg-green-50 border border-green-200 rounded-xl p-5 flex items-center gap-3">
        ...
        <div>
          <p class="font-semibold text-green-800">Semua tagihan lunas!</p>
          <p class="text-sm text-green-600">Tidak ada tagihan yang tertunggak saat ini.</p>
        </div>
      </div>
    {/if}
  {/if}
